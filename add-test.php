...
<?php
    ...
    if(!isset($_SESSION['login'])) {
        ...
        $image = "testing/".$_FILES["image"]["name"];
        if(move_uploaded_file($_FILES["image"]["tmp_name"],"testing/".$_FILES["image"]["name"])) {
            if(mysqli_query($conn, "update assign set test_file='$image' where id = '$_GET[id]'")) { 
                echo "<script>alert('Task submitted successfully..!');location.href='test.php';</script>";
            }
            else {

    ...
    if(!isset($_SESSION['login'])) {
        ...
        if(!empty($_FILES['image']['name'])) {
            ...
            if(move_uploaded_file($_FILES["image"]["tmp_name"],"user/".$_FILES["image"]["name"])) {
                if(mysqli_query($conn, "update admin set name = '$fname', email = '$email', contact = '$num', image = '$image' where username = '$username'")) {     
                    echo "<script>alert('Profile updated successfully..!');location.href='admin-profile.php'</script>";
                }
                else {
            ...
            if(mysqli_query($conn, "update admin set name = '$fname', email = '$email', contact = '$num' where username = '$username'")) {     
                echo "<script>alert('Profile updated successfully..!');location.href='admin-profile.php'</script>";
            }
            else {
                echo "<script>alert('Opps, Unable to update profile..!');</script>";
...
<section class="w3l-index3 py-3">
    <div class="container py-lg-3 mt-3">
        <h3 class="mb-3">Update Admin Profile</h2>
